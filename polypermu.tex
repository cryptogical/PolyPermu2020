...
$P$ est un polynôme de permutation si et seulement si les deux conditions suivantes ont lieu
\begin{enumerate}
...
def estDePermutation(f, q):
    ...
    S.<t> = PolynomialRing(R) # Anneau de polynômes sur F_q en l'indéterminées t
    f = S(f)
    image = []
    for i in R:
        temp = f(i)
...
print(" Les polynômes de permutations sur à coefficients dans F_" + str(q) + 
...
\caption{Affichage de l'éxécution}
\end{figure}
\pagebreak
Nous allons ensuite présenter une implémentation sous forme algorithmique le \textbf{Critère 1} présenté précédemment. Cet algorithme est développé en \textit{Sage}.
\begin{minted}
[
frame=lines,
framesep=2mm,
...
\caption{Affichage de l'éxécution}
\end{figure}
Cet algorithme nécessite plus de calculs que le précédent mais permet de donner une réponse directe à la question "Est ce que $P \in \mathbbm{F}_q [X]$ est de permutation ?". \#TODO: Donner la complexité de ces algorithmes.
\pagebreak
...
Cet algorithme est assez intéressant car il permet de tester directement un polynôme bien spécifique, plutôt que le premier qui lui permet de lister les polynômes de permutations de degré précisé.
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\pagebreak

Enfin nous allons présenter une implémentation du \textbf{Critère de Hermite-Dickson}. Cette implémentation a été réalisée elle aussi en \textit{Sage}.
...
Les méthodes d'identifications que nous venons de voir sont, fonctionnelles certes, mais non raffinées. Elles sont extrêmement couteuses en terme de puissance de calcul. Il est donc primordial d'établir une liste de critères permettant d'accélérer le processus. Cette partie y sera entièrement consacrée. Pour ce faire, nous travaillons sur des ''familles'' de polynômes. Nous n'aurons pas la prétention d'en donner une liste exhaustive; cependant dans ce premier rendu nous donnerons les plus élémentaires. Une étude plus approfondie sera faite au second semestre.
...
Un polynôme linéarisé correspond à une application linéaire de $\mathbb{F}_q$ dans lui-même et est de la forme $f(x) = \sum_{i=0}^{k} a_i x^{p^i} \in \mathbb{F}_{p^n}[x] $, où $\mathbb{F}_{p^n}[x] = \mathbb{F}_q$. Ce polynôme est de permutation si et seulement s'il admet 0 comme unique racine dans $\mathbb{F}_q$.\\
Beaucoup d'exemples de polynômes de permutations peuvent être générés à partir de ce type de polynômes, c'est pourquoi nous les introduisons ici avec les théorèmes suivants :\\
...
{\color{red} \textbf{Raph} : Je chercherai une démonstration correcte, au moins pour un des types de polynôme énoncés.}
