...
\section*{\huge{Notations}}
\vspace{36pt}
$\mathbb{F}_q$ \hspace{84pt} Corps de \textit{Galois} à $q$ éléments.\\
$\mathbb{F}_q$* \hspace{79pt} Ensemble des éléments inversibles de $\mathbb{F}_q$\\
$\overline{\mathbb{F}_p}$ \hspace{84pt} Clôture algébrique de $\mathbb{F}_p$.\\
$\mathbb{F}_q[X]$\hspace{71pt} Anneau des polynômes en l'indéterminée $X$ à coefficients dans $\mathbb{F}_q$.\\
$\mathbbm{Poly}$ \hspace{71pt} Ensemble des polynômes de permutation (définir sur quel corps).\\
$deg(f)$ \hspace{63pt} Degré du polynôme $f$.\\
$P(X) \wedge Q(X)$ \hspace{25pt} PGCD de $P$ et $Q$.\\
$\mathbb{F}_p(\alpha)$ \\
\pagebreak
...
\item Réciproquement, on suppose que $d$ divise $n$ et soit $P \in \mathcal{P} (n,p)$. Soit $\alpha$ une racine de $P$ dans le corps de rupture de $P$ sur $\F_p$. Alors par le théorème ? on a $\F_p (\alpha) \simeq \F_{p^d}$. D'où $\alpha$ est racine de $X^{p^n} - X$. Or, puisque $P$ est irréductible, alors $P$ est le polynôme minimal de $\alpha$ sur $\F_p$ donc $P$ divise $X^{p^n} - X$. En outre les facteurs irréductible de$X^{p^n} - X$ sur $\F_p$ sont simples puisque $P$ est le polynôme minimal de $\alpha$ et que $P$ divise $X^{p^n} - X$.
\end{itemize}
\end{proof}
\begin{coro}
...
\end{example}
Nous allons maintenant énoncer un non-critère fort, qui sera admis pour le moment. Sa démonstration s'appuie sur des éléments complexes de la théorie de \textit{Galois}. Nous nous laissons l'opportunité de le démontrer en ce second semestre.
...
\begin{proof}
Si $L$ est un monôme, alors $a=0$ convient. On admettra que les polynômes du type $\frac{L(X)}{X}$, où $L(X)$ est un polynôme linéarisé, ne sont de permutation que si ce sont des monômes. Il existe donc au moins une valeur $a$ qui n'est pas prise par ce polynôme, et le polynôme linéarisé $$X(\frac{L(X)}{X}-a) = L(X)-aX$$ n'a alors que $0$ comme racine et est donc de permutation.
\end{proof}
...
Les rappels sur les corps finis étaient primordiaux de manière à bien définir l'environnement sur lequel nous allions travailler, à savoir pour énoncer les premières définitions et propriétés des polynômes de permutation sur les corps finis. \\
L'étude des polynômes de permutations est un domaine vaste des mathématiques, notamment en raison des applications de ces polynômes, principalement en cryptologie. Nous avons ici tâché de nous en tenir aux parties essentielles des polynômes de permutation, de manière à ne pas saturer notre travail d'informations obsolètes, i.e, n'ayant pas de lien direct ou partiel avec la cryptologie.\\
Il est fréquent qu'un polynôme de permutation "compliqué" sur un corps finis représente en réalité une permutation simple. Et réciproquement, une permutation compliquée sera généralement représentable par un polynôme de permutation simple, ce qui en permettra une implémentation facile, donc une bonne application en cryptologie. Tout ceci se relie directement avec les différents critères et algorithmes présentés plus hauts. Ces derniers permettant de savoir quels polynômes sont de permutations et donc à fortiori de trouver de bons polynômes de permutation pour une application en cryptologie, notamment pour le crypto système RSA énoncé plus haut (\textbf{N.B :} Partie détaillée au semestre 2). Aujourd'hui, le réel problème est de trouver des algorithmes déterministes efficaces, c'est à dire au plus en temps polynomial, bien que les algorithmes actuels commencent à donner des résultats en temps satisfaisant.
\\
